...
매장의 성패가 결국 **시스템과 데이터를 얼마나 잘 다루는가**에 달려 있다는 사실을 매일 체감하며,
사용자가 아닌 **시스템을 직접 만드는 사람**이 되기로 결심했습니다.

2025년 12월, 안정적인 사업장을 아버지에게 인계하고 풀스택 국비 과정을 시작했습니다.
**오늘까지 5개월 동안 단 한 번의 결석·지각 없이** 모든 과정에 임했고,
팀 프로젝트와 개인 프로젝트를 모두 **끝까지 책임지고 완성**하며 풀스택 흐름을 처음부터 끝까지 체득했습니다.
...
**🛠 Tech**
`Spring Boot 3.x` `React+Vite` `Zustand`
`Three.js (R3F)` `MySQL` `Redis` `FAISS`
...
**문제** — HTTPS 프론트엔드 ↔ HTTP 백엔드 사이의
모든 API 요청을 브라우저가 차단

**해결** — 인증서 발급 일정이 빠듯해서
**CloudFront Behavior 규칙으로 EC2 프록시 구성**
→ 단일 HTTPS 도메인으로 통일

**배운 점** — 풀스택 개발자가 가져야 할 시야는
*인프라 + 프론트 + 백엔드를 함께 보는 눈*
...
**문제** — Seq2Seq LSTM 300 epoch 학습 후에도
858개 QA로는 답변 품질이 일관되지 않음

**해결** — 생성형 → **검색형 모델로 구조 전환**
TF-IDF 후보 20개 → KoBERT 의미 유사도 → 최종 선택
**하이브리드 구조 + 6개 카테고리 폴백 UX**

**배운 점** — 모델 선택은 무조건 최신 기술이 아니라
**데이터 양과 도메인 특성에 맞춰야 한다**
